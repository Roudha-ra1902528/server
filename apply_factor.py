...
def generate_images(z, label, truncation_psi, noise_mode, direction, file_name):
    if(args.space == 'w'):
        ...
        img1 = G(z, label, truncation_psi=truncation_psi, noise_mode=noise_mode, force_fp32=True)
        img2 = G(z + direction, label, truncation_psi=truncation_psi, noise_mode=noise_mode, force_fp32=True)
        img3 = G(z - direction, label, truncation_psi=truncation_psi, noise_mode=noise_mode, force_fp32=True)
...
if __name__ == "__main__":
    ...
    with dnnlib.util.open_url(args.ckpt) as f:
        G = legacy2.load_network_pkl(f, custom=custom, **G_kwargs)['G_ema'].to(device) # type: ignore
